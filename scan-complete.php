...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ====== PAGE TOPBAR ====== */
    .page-topbar { display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 0; margin-bottom: 1.5rem; }
    .back-btn { display: flex; align-items: center; justify-content: center; width: 44px; height: 44px; border-radius: 50%; border: 1px solid var(--gold); color: var(--gold); text-decoration: none; transition: all var(--transition); background: transparent; flex-shrink: 0; }
    ...
    /* ====== LAYOUT ====== */
    .main { max-width: 100%; margin: 0 auto; padding: 0 3rem 2.5rem; display: flex; flex-direction: column; gap: 1.75rem; align-items: center; text-align: center; }

    /* ====== HEADER ====== */
    .page-label { font-size: 0.6rem; letter-spacing: 0.25em; text-transform: uppercase; color: var(--gold); font-weight: 500; margin-bottom: 0.2rem; }
    .page-title { font-family: var(--font-display); font-style: italic; font-size: 2rem; font-weight: 400; color: var(--white); line-height: 1.05; }
    ...
    .detect-overlay.hidden { opacity: 0; pointer-events: none; }
    ...
    .detect-dots { display: flex; gap: 0.4rem; }
    .detect-dots span {
      width: 7px; height: 7px; border-radius: 50%; background: #2a2a22;
      animation: pulseDot 1.4s ease-in-out infinite both;
    }
    .detect-dots span:nth-child(1) { animation-delay: -0.32s; }
    .detect-dots span:nth-child(2) { animation-delay: -0.16s; }
    .detect-dots span:nth-child(3) { animation-delay: 0s; }

    @keyframes pulseDot {
      0%,80%,100% { transform:scale(0.5); opacity:0.2; background:#2a2a22; }
      40% { transform:scale(1); opacity:1; background:var(--gold); }
    }
    ...
    /* ====== DATA CARDS ====== */
    .data-cards-row { display: flex; gap: 1rem; width: 100%; max-width: 360px; }
    .data-card { flex: 1; background: #161610; border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 1rem 1.25rem; text-align: center; }
    .data-card-label { font-size: 0.55rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--white-dim); font-weight: 500; margin-bottom: 0.25rem; }
    .data-card-value { font-family: var(--font-display); font-size: 1.2rem; font-weight: 400; color: var(--white); line-height: 1.2; }

    /* ====== RECOMMENDED ====== */
    .recommended-section { width: 100%; max-width: 360px; }
    .rec-section-label { font-size: 0.6rem; letter-spacing: 0.15em; text-transform: uppercase; color: var(--gold); font-weight: 500; margin-bottom: 1rem; text-align: center; }
    .rec-buttons-row { display: flex; gap: 0.75rem; justify-content: center; flex-wrap: wrap; }
    .rec-btn { padding: 0.6rem 1.4rem; border-radius: 50px; border: 1px solid var(--border); background: #161610; color: var(--white-dim); font-size: 0.7rem; letter-spacing: 0.05em; transition: all var(--transition); flex: 1; min-width: 80px; }
    .rec-btn:hover { border-color: var(--border-hover); color: var(--white); }
    .rec-btn.active { border-color: var(--gold); background: var(--gold-muted); color: var(--gold); font-weight: 500; }

    /* ====== ACTIONS ====== */
    .actions { display: grid; gap: 0.75rem; width: 100%; max-width: 360px; }
    .btn { display: inline-flex; align-items: center; justify-content: center; width: 100%; padding: 0.85rem 1.2rem; border-radius: 999px; font-size: 0.75rem; letter-spacing: 0.12em; text-transform: uppercase; font-weight: 600; text-decoration: none; transition: all var(--transition); cursor: pointer; border: none; font-family: var(--font-body); }
    ...
    .secondary-link { margin-top: 0.25rem; font-size: 0.7rem; color: var(--gold); cursor: pointer; transition: all var(--transition); font-weight: 400; letter-spacing: 0.05em; }
    .secondary-link:hover { opacity: 0.7; }
    .rescan-link { font-size: 0.65rem; color: var(--white-dim); cursor: pointer; letter-spacing: 0.05em; transition: color var(--transition); }
    .rescan-link:hover { color: var(--gold); }
    ...
    .fade-6 { animation: fadeUp 0.55s ease 0.55s both; }
    ...
    <div class="face-card fade-3">
      ...
      <div class="card-badge-row">
        <div class="card-badge">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0a0a08" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round">
...
const kps = landmarks.map(p => ({
  ...
  localStorage.setItem('scanAnalysis', JSON.stringify({ browShape, faceShape, recos }));
}
